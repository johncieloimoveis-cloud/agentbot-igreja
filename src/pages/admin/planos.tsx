...
import { Building2, Crown, Gift, RefreshCw, Users, Check, Pencil } from 'lucide-react';
...
export default function AdminPlanos() {
  ...
  useEffect(() => {
    if (editandoLimite) inputRef.current?.focus();
  }, [editandoLimite]);
  ...
  const iniciarEdicaoLimite = (igreja: Igreja) => {
    ...
    setLimiteInput(String(igreja.people_limit ?? 150));
  };
  ...
  const salvarLimite = async (id: string) => {
    ...
    if (res.ok) {
      setIgrejas(prev => prev.map(i => i.id === id ? { ...i, people_limit: limit } : i));
    }
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700 flex items-center gap-3">
                ...
                {editandoLimite === ig.id ? (
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <button
                      onClick={() => salvarLimite(ig.id)}
                      disabled={atualizando === ig.id + '_limit'}
                      className="p-1.5 bg-primary-600 hover:bg-primary-700 text-white rounded-lg disabled:opacity-50"
                      title="Salvar"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => iniciarEdicaoLimite(ig)}

...
function NivelBadge({ nivel }: { nivel: string }) {
  const cfg: Record<string, string> = {
    ...
    'Média': 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300',
...
function RecomendacaoBadge({ rec }: { rec: string }) {
  const isPublic = rec === 'Culto público' || rec === 'Ambos';
...
export default function AnaliseletraPage() {
  ...
  const toggleDim = (i: number) =>
    setExpandidas(prev => ({ ...prev, [i]: !prev[i] }));

  const notaGeral = analise?.nota_geral ?? 0;
  const notaCor =
    notaGeral >= 7 ? 'text-emerald-600 dark:text-emerald-400' :
    notaGeral >= 4 ? 'text-amber-600 dark:text-amber-400' :
    'text-red-600 dark:text-red-400';
  const notaRing =
    notaGeral >= 7 ? 'ring-emerald-400' :
    notaGeral >= 4 ? 'ring-amber-400' :
    'ring-red-400';
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5 space-y-4 shadow-sm">
        <div>
          ...
          <input
            type="text"
            placeholder="Ex: Oceans — Hillsong United"
            value={titulo}
        ...
        <div>
          ...
          />
          {letra && (
            <p className="text-xs text-gray-400 mt-1 text-right">{letra.length} caracteres</p>
          )}
        </div>
        ...
        <button
          onClick={analisar}
          disabled={analisando || !letra.trim()}
      ...
      {analise && (
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-2">
                {analise.titulo_detectado && (
                  <p className="text-xs text-gray-400 dark:text-gray-500 italic">"{analise.titulo_detectado}"</p>
                )}
                ...
                <div key={i} className="px-5 py-4">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-bold ${
                          d.nota >= 7 ? 'text-emerald-600 dark:text-emerald-400' :
                          d.nota >= 4 ? 'text-amber-600 dark:text-amber-400' :
                          'text-red-600 dark:text-red-400'
                        }`}>{d.nota.toFixed(1)}</span>
                  ...
                  {expandidas[i] && (
                    <div className="mt-3 space-y-2">
                      ...
                      {d.trecho && (
                        <blockquote className="border-l-2 border-violet-400 pl-3 text-sm italic text-gray-500 dark:text-gray-400">
                          "{d.trecho}"
                        </blockquote>
                      )}
                    </div>
          ...
          {/* Nova análise */}
          <button
            onClick={() => { setAnalise(null); setLetra(''); setTitulo(''); setErro(''); }}

...
export default function Biblia() {
  ...
  const carregar = async (bookId: string, cap: number) => {
    ...
    try {
      const res = await fetch(
        `https://bible-api.com/${bookId}+${cap}?translation=almeida`
      );
  ...
  const anterior = () => {
    if (capitulo > 1) {
      irPara(livroId, capitulo - 1);
    } else {
      const idx = TODOS.findIndex(l => l.id === livroId);
      if (idx > 0) irPara(TODOS[idx - 1].id, TODOS[idx - 1].cap);
    }
  };

  const proximo = () => {
    if (capitulo < livro.cap) {
      irPara(livroId, capitulo + 1);
    } else {
      const idx = TODOS.findIndex(l => l.id === livroId);
      if (idx < TODOS.length - 1) irPara(TODOS[idx + 1].id, 1);
    }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">João Ferreira de Almeida · Domínio Público</p>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <select
          value={livroId}
          onChange={e => irPara(e.target.value, 1)}
        ...
        <select
          value={capitulo}
          onChange={e => irPara(livroId, Number(e.target.value))}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <button
              onClick={() => carregar(livroId, capitulo)}
          ...
          <>
            <h2 className="text-lg font-bold text-primary-600 dark:text-primary-400 mb-6 border-b border-gray-100 dark:border-slate-700 pb-3">
              {referencia}
            </h2>
            <div className="space-y-4 text-gray-800 dark:text-gray-200 leading-relaxed text-[15px]">
              {verses.map(v => (
                <p key={v.verse} className="flex gap-2">
                  <span className="text-primary-400 font-bold text-xs mt-1 w-5 flex-shrink-0 text-right select-none">
                    {v.verse}
                  </span>
      ...
      <div className="flex justify-between pb-4">
        <button
          onClick={anterior}
          disabled={livroId === 'genesis' && capitulo === 1}
        ...
        <button
          onClick={proximo}
          disabled={livroId === 'revelation' && capitulo === 22}
          className="flex items-center gap-2 px-5 py-2.5 border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors disabled:opacity-30"
        >
          Próximo

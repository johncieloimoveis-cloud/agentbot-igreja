...
export default function PublicQuizBiblicoPage() {
  ...
  const iniciar = async () => {
    ...
    try {
      ...
      setAtual(0); setRespondida(false); setTela('jogando');
    } catch (err: any) {
      setErro(err.message || 'Erro ao carregar perguntas'); setTela('inicio');
    }
  };
  ...
  const responder = (i: number) => {
    ...
    setRespondida(true);
    const novas = [...respostas]; novas[atual] = i; setRespostas(novas);
  };

  const proxima = () => {
    if (atual + 1 >= perguntas.length) { setTela('resultado'); }
    else { setAtual(atual + 1); setRespondida(false); }
  };

  const reiniciar = () => { setTela('inicio'); setPerguntas([]); setRespostas([]); setAtual(0); setRespondida(false); };
      ...
      {tela === 'inicio' && (
        <div className="max-w-xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIAS.map(c => (
                <button key={c.id} onClick={() => setCategoria(c.id)}
                  className={`text-left p-3 rounded-xl border transition-all ${categoria === c.id ? 'border-amber-400 bg-amber-50 dark:bg-amber-900/30 dark:border-amber-500' : 'border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              {DIFICULDADES.map(d => (
                <button key={d.id} onClick={() => setDificuldade(d.id)}
                  className={`p-3 rounded-xl border text-center transition-all ${dificuldade === d.id ? 'border-amber-400 bg-amber-50 dark:bg-amber-900/30 dark:border-amber-500' : 'border-gray-200 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700'}`}>
          ...
          <button onClick={iniciar}
            ...
            <Sparkles className="w-5 h-5" />Iniciar Quiz
          </button>
        </div>
      )}
      ...
      {tela === 'jogando' && perguntas.length > 0 && (() => {
        ...
        return (
          <div className="max-w-xl mx-auto space-y-5">
            ...
            {respondida && (
              <div className={`rounded-xl p-4 text-sm ${respostas[atual] === p.correta ? 'bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-300'}`}>
                <p className="font-semibold mb-1">{respostas[atual] === p.correta ? '✅ Correto!' : '❌ Incorreto'}</p>
            ...
            {respondida && (
              <button onClick={proxima}
                ...
                {atual + 1 >= perguntas.length ? <><Trophy className="w-5 h-5" />Ver resultado</> : <>Próxima <ChevronRight className="w-5 h-5" /></>}
              </button>
            )}
          </div>
        );
          ...
          <div className="max-w-xl mx-auto space-y-5">
                ...
                {perguntas.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="px-5 py-3 flex items-start gap-3">
                      {acertou ? <CheckCircle2 className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" /> : <XCircle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={iniciar}
                ...
                <Sparkles className="w-4 h-4" />Repetir tema
              </button>
            </div>
          </div>
        );
